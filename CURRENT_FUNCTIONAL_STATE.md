...
- Beginner visual dashboard UX layer implemented/verified: ChatGPT mini card, Codex execution card и полный Markdown dashboard рендерятся из одного `project-lifecycle-dashboard.yaml`; validator блокирует false no-user-action при `external_actions_ledger`, Codex completed/executed без evidence, false auto-switch и heavy default UI promise
...
- standards-based lifecycle navigator/control layer добавлен как обязательный repo-native слой для `factory-template` и generated projects: registry, lifecycle stage map, generated `standards-gates.yaml`, dashboard integration, offline watchlist, validators, docs and beginner status line
- standards navigator связывает ISO/IEC/IEEE 12207, Scrum Guide/Kanban-lite, ISO 25010, NIST SSDF, OWASP ASVS, WCAG 2.2, DORA metrics and AI-specific safety overlay with lifecycle phases and evidence gates, without claiming formal certification/compliance
- lifecycle dashboard теперь показывает selected standards profile, lifecycle backbone version/status, standards gate summary, current phase required standards, missing standards evidence, next safe standards action, monitoring status and `allowed_to_advance_phase`
- validators теперь блокируют standards false green: production/commercial claim with solo profile only, security/accessibility green without evidence, AI readiness without `ai_safety_gate`, stale standard overclaim and certification/compliance claim without evidence
- controlled software update governance добавлен как обязательный repo/template/generated слой: policy/spec, inventory, watchlist, readiness, reports, dashboard readout, validator и report-only renderer.
- VPS/Ubuntu operate path теперь фиксирует selected Ubuntu LTS release / provider image id, OS image release отдельно от later package update state, `unattended-upgrades`, package sources, Docker/Compose, Node/Python, GitHub Actions, base images/tags/digests, lockfiles и critical runtime dependencies.
- default update policy для generated projects: `manual-approved-upgrade`; auto-install/auto-upgrade без explicit approval пользователя запрещен, а переход на новую Ubuntu LTS считается отдельным migration/upgrade project.
...
- граница evidence: `2.5.7 Package Ready` означает, что repo-controlled GA evidence линии `2.5` сохраняется, а patch release добавляет проверяемый Windows beginner latest install guide order contract
...
- `2.5.7` scope: Windows beginner one-file latest install guide order fix: VPS/Windows preparation before executable launch, dynamic GitHub Release download, checksum verification, recursive installer discovery, passwordless SSH setup retained, archive fallback and npm-unsupported boundary.
