...
- граница evidence: `2.5.6 Package Ready` означает, что repo-controlled GA evidence линии `2.5` сохраняется, а patch release добавляет проверяемый Windows beginner latest install guide artifact contract
...
- `2.5.6` scope: Windows beginner one-file latest install guide with dynamic GitHub Release download, checksum verification, recursive installer discovery, passwordless SSH setup retained, archive fallback and npm-unsupported boundary.
