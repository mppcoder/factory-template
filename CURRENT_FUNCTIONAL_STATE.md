...
- copy-block layer теперь требует, чтобы любой предложенный конкретный текст для вставки, отправки, instruction, UI-поля, чата или CLI выдавался отдельным fenced code block для копирования, даже если это не финальный внешний шаг
...
- direct task Codex first answer теперь начинается с `Номер запроса Codex` (`FT-CX-....`) и `Карточка проекта` из repo renderer до route receipt/remediation
...
- Module Gate Baseline is materialized without false-green: Lifecycle/Core/Security/Quality/WebSec/UI-A11y have repo evidence, Ops is `completed` after report-only controlled software update governance baseline recording, AI remains `not_applicable`, and downstream/battle proof now has external OpenClaw local runtime pilot evidence while public HTTPS/nginx proof remains `not_claimed`.
...
- optional future public endpoint proof: downstream/battle public HTTPS/nginx proof still requires separate approval and evidence. OpenClaw external pilot (`mppcoder/openclaw`) has local prod runtime proof with real image, deploy, localhost healthcheck, backup, restore, rollback, source hardening and operator readiness.
...
- Plan №4 status: P4-S0..P4-S4 prepared; OpenClaw local runtime pilot evidence recorded; public endpoint proof remains not claimed with validator-backed false-pass guards for placeholder image, missing restore/rollback and secret-like transcript.
- Recommended next roadmap branch: preserve OpenClaw local runtime evidence and continue reusable template feedback, then proceed to beginner-first Windows-to-first-project hardening. Public HTTPS/reverse-proxy proof is optional future work and requires explicit approval.
...
- downstream/battle application local runtime proof has external OpenClaw evidence; public endpoint proof still requires separate domain/TLS/nginx approval and sanitized transcript.
- Plan №4 public proof requires external inputs: public endpoint boundary, domain/TLS/nginx approval, exposed-port decision and public healthcheck evidence.
- Если public endpoint inputs отсутствуют, не симулировать public pass; удерживать `public_https: not_claimed` и продолжать beginner-first hardening.
- Real package/runtime upgrades remain future approval work: текущий software governance baseline только записывает inventory/watchlist/readiness, а `apt upgrade`, `apt install`, Docker image upgrade, dependency upgrade и production deploy не выполнялись.
