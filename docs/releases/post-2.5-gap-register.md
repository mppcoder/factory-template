...
| `P25-GAP-05` | Backup restore and rollback drill are documented but not executed on real runtime. | `reports/release/2.6-runtime-proof-report.md` records backup, disposable restore and rollback drill. | `2.6` | remediated for infrastructure proof | real VPS / restore target |
...
| `P3-GAP-05` | Pre/post deploy QA must become an explicit runtime proof boundary. | Related to `P25-GAP-04` and `P25-GAP-05`; P3-S5 prepared the boundary, and 2.6 runtime proof executed deploy/healthcheck/backup/restore/rollback for infrastructure/demo app image. | P3-S5 / 2.6 | executed for infrastructure proof | external runtime |
