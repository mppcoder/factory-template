...
Когда Codex или ChatGPT передает внешний шаг пользователю, это должен быть диалоговый сценарий: коротко объяснить цель, назвать окно, разложить действие по шагам, предложить варианты с рекомендованным вариантом и fallback там, где развилка реальна, и описать ожидаемый результат. Если пользователь должен копировать текст, команду, handoff или repo-first instruction, runbook/closeout должен выдавать готовый fenced code block; ссылку на файл можно использовать только как evidence, а не как замену текста для копирования.
...
## Default-decision layer для intake/questionnaire

Beginner questionnaires must be recommendation-first. Blank expert-only questions are forbidden if safe defaults exist.

Каждый intake/questionnaire flow обязан поддерживать два пользовательских режима:

- `global-defaults mode`: в начале опроса ChatGPT Project спрашивает: "Использовать рекомендуемые решения по умолчанию на основе лучших доступных практик, концепции и масштаба проекта?" Варианты: принять safe defaults и спрашивать только реальные choices; подтверждать каждое recommended default отдельно; задавать все решения вручную.
- `per-question-default mode`: каждый вопрос показывает вопрос, recommended default, почему он подходит в 1-2 короткие строки, что будет сделано при Enter/`по умолчанию`, и как указать свой вариант.

Every default must be explainable and overrideable. Default source basis фиксируется как `repo-policy`, `official-docs`, `best-practice`, `project-scale` или `user-override`; если рекомендация требует свежей информации, она помечается `requires fresh check` или уходит в `uncertainty_notes`.

Обязательные поля generated intake/session artifacts и generated handoff:
...
Risky, paid, destructive, security, privacy, legal, secret-related decisions still require explicit user confirmation. Такие решения нельзя silently default или auto-accept даже в `global-defaults mode`.
