...
Пользователь предоставляет existing repo access/path и approvals. Codex выполняет audit/adoption/conversion. Intake работает в recommendation-first режиме: safe defaults объясняются и могут быть заменены.
Если existing проект уже работает на VPS, пользователь предоставляет только безопасные факты о provider image/Ubuntu release и approvals; secrets и `.env` content не копируются в repo. Codex не выполняет auto-upgrade: сначала фиксируется current baseline, update watchlist и readiness.
...
- Где взять значения: `<repo-owner>/<repo-name>` взять из GitHub URL; `<project-slug>` обычно repo name. Если пользователь не просит иначе, default-decision layer рекомендует keep existing repo as canonical root, do not overwrite product-owned code, evidence-first audit before remediation, затем conversion в `greenfield-product` или documented blocker.
...
decisions_requiring_user_confirmation:
  - protected branch write/merge approval
...
- Ожидаемый результат: Один canonical project root определен; defaults accepted or overridden; custom overrides captured.
