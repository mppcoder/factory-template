...
Если change затрагивает runbook layer, scenario-pack, launcher, validators, codex-task-pack или другой downstream-consumed template content, Codex сначала должен определить, есть ли реальные внешние действия пользователя.
Если действий нет, инструкция должна быть короткой: `Внешних действий не требуется.`
Если действия есть, инструкция расширяется до компактного completion package только по этим действиям и затронутым контурам:
...
1. раздел `Название чата для копирования` со stable title из repo `.chatgpt/chat-handoff-index.yaml` / allocator;
2. раздел `Карточка проекта` со свежей compact card из repo dashboard.

Если точный следующий номер неизвестен, не придумывай его. Напиши: `Нужно выделить номер через repo chat-handoff-index / allocator.`
...
- синхронизация `template-repo/scenario-pack/`, `template-repo/scripts/`, `factory/producer/reference/examples/` и generated project artifacts;
