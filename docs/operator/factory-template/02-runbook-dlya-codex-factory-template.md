...
Если внешний шаг связан с обновлением repo-first инструкций или downstream sync flows, сначала проверь, есть ли реальное действие пользователя.
Если действий нет, не собирай расширенный completion package; напиши `Внешних действий не требуется.`
Если действия есть, не ограничивайся общим footer. Собирай компактный completion package только по этим действиям с секциями:
- `Что изменено`
...
2. Найти основные слои:
   ...
   - generated project `.chatgpt/` artifacts внутри `template-repo/template/`, `factory/producer/reference/examples/`, `.matrix-test/` и `.smoke-test/`
...
- `factory/producer/reference/examples/` — golden examples и content fixtures
- `project-knowledge/factory/template-evolution/` — release notes и backlog самой фабрики
...
- после handoff новая задача Codex должна стартовать через launcher/profile flow, а не через старую закрепленную сессию.
