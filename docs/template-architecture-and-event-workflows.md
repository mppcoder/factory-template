...
- Для `greenfield-product` пользователь не создает GitHub repo, не добавляет `origin`, не делает first push и не запускает launcher/verify. User-only boundary: сказать `новый проект` в ChatGPT Project шаблона фабрики, ответить на опрос, вставить generated handoff в Codex, затем создать battle ChatGPT Project и вставить готовую repo-first инструкцию, подготовленную Codex.
