...
- Если найден unresolved incidental defect, сначала подготовить bug report и self-handoff именно для него, а не считать текущую live-сессию уже автоматически перенастроенной.
- Если incidental defect требует другого route, канонический вариант для пользователя: скопировать новый handoff и запустить новый Codex task/chat.
- Продолжение в текущем чате допустимо только как явно помеченный fallback с оговоркой, что auto-switch profile/model/reasoning в уже открытой сессии ненадежен.
- Если defect требует глубокого исследования, вместо weak remediation attempt выдать ChatGPT-ready research prompt.
...
## User-facing wording for incidental defects

Используйте готовые формулировки:

1. Продолжить в текущем чате на текущем route
   `Self-handoff для incidental bug подтверждает тот же profile/model/reasoning. Можно продолжить в текущем чате, но только потому, что route совпал; это не результат автоматического переключения сессии.`

2. Рекомендуемый путь: новый Codex task/chat
   `Self-handoff для incidental bug выбрал другой route. Канонический путь — скопировать новый handoff ниже и запустить новый Codex task/chat; уже открытая сессия не является надежным механизмом auto-switch profile/model/reasoning.`

3. Deep research
   `Текущих данных недостаточно для надежной remediation-попытки в этой сессии. Ниже подготовлен ChatGPT-ready bug report/prompt для deep research; это канонический следующий шаг вместо слабого in-session фикса.`
