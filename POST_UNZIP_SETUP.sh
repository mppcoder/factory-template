...
for f in launcher.sh SMOKE_TEST.sh EXAMPLES_TEST.sh MATRIX_TEST.sh PRE_RELEASE_AUDIT.sh RELEASE_BUILD.sh CLEAN_VERIFY_ARTIFACTS.sh EXPORT_FACTORY_TEMPLATE_SOURCES.sh GENERATE_BOUNDARY_ACTIONS.sh VALIDATE_FACTORY_TEMPLATE_OPS.sh VALIDATE_FACTORY_FEEDBACK.sh INGEST_FACTORY_FEEDBACK.sh TRIAGE_INCOMING_LEARNINGS.sh DETECT_FACTORY_TEMPLATE_PHASE.sh PHASE_DETECTION_TEST.sh; do chmod +x "$ROOT/$f" 2>/dev/null || true; done
