# Русское ядро фабрики проектов v2.5.8
...
Готовый файл для нового пользователя с latest-ссылками и copy-paste блоками:

```text
WINDOWS_INSTALL_LATEST.md
```

Он скачивает latest release через GitHub API, проверяет SHA256, распаковывает ZIP и запускает текущий executable path `windows-bootstrap/install-windows.ps1` без ручного угадывания версии.
...
Bootstrapper проверяет `ssh.exe`/`scp.exe`, по возможности находит `git.exe` и `code.exe`, спрашивает VPS host/IP, SSH username и port, создает `/projects/factory-template/_incoming`, рекомендует GitHub clone/download из `mppcoder/factory-template`, поддерживает fallback archive upload `factory-v2.5.8.zip` + manifest + SHA256, запускает remote quick verification и сохраняет install log/next-step report.
...
# загрузите factory-v2.5.8.zip, factory-v2.5.8.manifest.yaml и factory-v2.5.8.zip.sha256 в _incoming
cd /projects/factory-template/_incoming
sha256sum -c factory-v2.5.8.zip.sha256
rm -rf /projects/factory-template/factory-v2.5.8
unzip -q factory-v2.5.8.zip -d /projects/factory-template
cd /projects/factory-template/factory-v2.5.8
...
Archive contract: внутри zip должен быть один root `factory-v2.5.8/`; sidecar manifest `factory-v2.5.8.manifest.yaml` и checksum `factory-v2.5.8.zip.sha256` лежат рядом с archive.
...
## Что нового в релизе 2.5.8
- `WINDOWS_INSTALL_LATEST.md` выровнен по executable boundary: до запуска только PowerShell 7, скачивание/проверка latest package и запуск `install-windows.ps1`;
- SSH key setup, existing-key check, подключение к VPS, remote install и Codex prompt остаются автоматизацией installer;
- после вставки handoff дальнейшие repo-first шаги описаны как Codex-owned, а не user-run commands;
- fallback archive имена синхронизированы под `factory-v2.5.8.zip` + manifest + SHA256;
...
- `WINDOWS_INSTALL_LATEST.md` перестроен в правильном порядке: download/checksum/unzip отделен от запуска `windows-bootstrap/install-windows.ps1`;
...
- ZIP, manifest и SHA256 скачиваются вместе, checksum проверяется до распаковки;
- installer ищется рекурсивно как `windows-bootstrap/install-windows.ps1`, поэтому не нужно угадывать имя папки;
- fallback archive имена синхронизированы под `factory-v2.5.6.zip` + manifest + SHA256;
- `FactoryTemplateSetup.exe` по-прежнему future signed wrapper boundary, готовый exe не публикуется;
- npm path явно не заявляется, потому что repo не содержит `package.json`.

## Что нового в релизе 2.5.5
- Windows bootstrapper проверяет существующий SSH key через `ssh -o BatchMode=yes -i ...` до изменения VPS `authorized_keys`;
- если ключ уже работает, installer пропускает password step и не трогает `authorized_keys`;
- если private key есть, но `.pub` потерян, public key восстанавливается через `ssh-keygen.exe -y`;
- после добавления public key installer повторно проверяет key login;
- fallback archive имена синхронизированы под `factory-v2.5.5.zip` + manifest + SHA256.
