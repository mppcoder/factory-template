...
def main() -> int:
    ...
    ensure_contains(boundary, "диалоговый сценарий", errors, "boundary-actions.md")
    ensure_contains(boundary, "подробными пояснениями шагов", errors, "boundary-actions.md")
    ensure_contains(boundary, "рекомендованным вариантом и fallback-вариантом", errors, "boundary-actions.md")
    ensure_contains(boundary, "fenced code block", errors, "boundary-actions.md")
    ensure_contains(boundary, "не предлагай вместо этого перейти по ссылке на файл", errors, "boundary-actions.md")
    ...
    ensure_contains(boundary, "Каждый финальный ответ обязан содержать continuation outcome", errors, "boundary-actions.md")
    ensure_contains(boundary, "Следующий пользовательский шаг отсутствует; задачи текущего scope выполнены полностью.", errors, "boundary-actions.md")
    ensure_contains(boundary, "future boundary", errors, "boundary-actions.md")
    ensure_contains(boundary, "рекомендованную ветку и fallback-ветку", errors, "boundary-actions.md")
    ensure_contains(boundary, "не заменяет next-roadmap recommendation", errors, "boundary-actions.md")
    ...
    ensure_contains(checklist, "рекомендованную ветку и fallback-ветку", errors, "done-checklist.md")
    ensure_contains(checklist, "не заменяет ее фразой `Следующий пользовательский шаг отсутствует`", errors, "done-checklist.md")
    ensure_contains(checklist, "continuation outcome", errors, "done-checklist.md")
    ensure_contains(checklist, "Следующий пользовательский шаг отсутствует; задачи текущего scope выполнены полностью.", errors, "done-checklist.md")
    ensure_contains(checklist, "`## Инструкция пользователю` оформлена как диалоговый сценарий", errors, "done-checklist.md")
    ensure_contains(checklist, "fenced code block для копирования", errors, "done-checklist.md")
    ensure_contains(checklist, "Все тексты/команды/instruction/handoff, которые нужно копировать", errors, "done-checklist.md")
    ...
    ensure_contains(checklist, "задачи текущего scope выполнены полностью", errors, "done-checklist.md")
