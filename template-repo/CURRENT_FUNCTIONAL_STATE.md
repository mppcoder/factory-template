...
- advanced automation substrate включает dry-run issue-autofix proof, permission/label gates, bounded runner, worktree manager, permission enforcement, separate approval layer, hash-chain audit ledger, rollback proof, required human review policy, curator promotion flow и downstream sync guidance без full autonomous mode by default
...
- issue-autofix verification не мутирует live GitHub labels/issues; auto-merge, production deploy, security issue autofix, public external submit, security/external-secret/blocked tasks and full autonomous mode остаются human-gated/refused
