...
- цель изменения;
- обязательный раздел `goal_contract` с `normalized_goal`, `definition_of_done`, `evidence_required`, `scope`, `non_goals`, safety/budget boundaries, proxy-signal denylist и `goal_achievement_rule`;
- `goal_runtime_recommendation`: `goal_first_contract_only`, `manual_review`, `codex_goal_candidate`, `codex_goal_enabled`, `requires_feedback_setup` или `not_recommended`;
...
- нормализованное поле `handoff_shape` со значением `codex-task-handoff`.

`goal first` в handoff является mandatory template contract. `Codex /goal runtime` optional: его можно рекомендовать как рабочий runtime только после live validation текущего CLI/feature flags и явного включения/выбора. Если `goals` виден как experimental/off by default, handoff может фиксировать `codex_goal_candidate`, но не должен утверждать, что `/goal` уже активен.

Handoff нельзя делить на "goal command" и "остальной текст". Даже если runtime `/goal` доступен, пользователь получает один цельный copy-paste handoff block; repo-native launcher mode для goal может появиться позже, но пока это не причина дробить handoff.

Handoff запрещено закрывать proxy signals alone:
- tests passed alone;
- file exists alone;
- commit exists alone;
...
- `handoff_shape`: `codex-task-handoff` для новых handoff; legacy `single-agent-handoff` / `parent-orchestration-handoff` допускаются только при чтении старых артефактов;
...
Перед первым substantive ответом в новом ChatGPT task chat handoff должен получить stable id из repo counter:
...
- repo-first instruction authorizes configured GitHub connector: для `mppcoder/factory-template` repo-first instruction является prior authorization на настроенный authenticated GitHub connector / repo tool для обязательного repo read, index read и connector-safe reservation attempt;
- не спрашивай conversational confirmation перед чтением GitHub repo, чтением index или allocation attempt; вопросы вроде "подтвердите доступ к GitHub", "разрешите использовать GitHub" и "do you confirm GitHub access" запрещены до materialized allocation или confirmed blocker;
- platform-level OAuth / connector authorization prompt нельзя подавить repo instructions: если платформа требует authorization, назови `external_auth_blocker`; если write action exposed отсутствует или write rejected, назови `write_auth_blocker`; не заменяй connector attempt свободным вопросом пользователю;
- GitHub write-access request gate сохраняет ChatGPT write-access request mechanism перед allocator blocker: structured write-access request — not a conversational confirmation. Запрещен свободный вопрос о разрешении использовать GitHub до repo-first read/index read, но разрешен и обязателен structured write-access request, если он нужен для записи `.chatgpt/chat-handoff-index.yaml` и platform/connector может запросить write scope;
- порядок gate: repo-local allocator, если исполним; connector-safe reservation, если write action exposed; structured GitHub write-access request, если write action not exposed but request/escalation possible; retry connector-safe reservation after grant; exact allocator blocker только если `write_access_request_attempted` завершился как `request unavailable/rejected`, request rejected, materialized write failed after request, write action truly absent with no platform request path, write rejected или confirm fetch failed;
- если write action exposed and confirm fetch succeeds, exact allocator blocker запрещен и нужно показать materialized stable title;
...
- если ChatGPT не имеет write-доступа к repo/index или не может подтвердить запись, нельзя выдумывать номер; нужно сказать: `Нужно выделить номер через repo chat-handoff-index / allocator.`
- если ChatGPT handoff приходит в Codex с `chat_id: write_auth_blocker_current_chat`, Codex обязан первым remediation шагом retro-materialize ChatGPT reservation в `.chatgpt/chat-handoff-index.yaml` по фактическому `next_chat_number`, не расходуя Codex work counter и не придумывая dry-run номер;
- blocker нельзя выводить, когда GitHub connector write path доступен и confirm fetch подтверждает update; в этом случае нужно показать materialized `FT-CH-.... <task-slug>`;
- visible title/blocker под `Название чата для копирования` всегда должен быть отдельный однострочный fenced `text` code block, чтобы ChatGPT UI показал copy button и оператор мог скопировать значение одним кликом;
- инвариант stable identity: first substantive answer обязан дать materialized allocation or allocator blocker до route receipt, анализа или handoff;
- третье состояние запрещено: no allocation attempted / no blocker / answer continues;
- для ChatGPT-generated handoff route отсутствие allocation attempt/blocker до handoff является blocker/defect, а не допустимым fallback;
- `chatgpt-first-answer-allocation-not-attempted` является contract violation / defect-class и должен фиксироваться через defect-capture path;
- если handoff не запущен в Codex, записанный `chat_id` остается занятым; следующий ChatGPT task chat обязан брать новый номер, а старый можно только закрыть явным status update.

Первый ChatGPT-ответ по handoff/task обязан вывести перед handoff-блоком:

````markdown
## Название чата для копирования
```text
<chat_title или blocker allocator>
```

## Карточка проекта
<compact card из render-project-lifecycle-dashboard.py --format chatgpt-card --stdout>
````

Если renderer недоступен из ChatGPT, используй свежий `reports/project-status-card.md` или compact card из `reports/project-lifecycle-dashboard.md`. Нельзя молча опустить карточку: если card artifact недоступен, назови blocker явно.

Codex self-handoff не использует ChatGPT `FT-CH` counter и не занимает номера пользовательских ChatGPT task chats. Для прямых Codex-доработок используется отдельный repo counter:

- source of truth: `.chatgpt/codex-work-index.yaml`;
- command path для repo executor: `python3 template-repo/scripts/allocate-codex-work-id.py --kind self_handoff --description "<short task>"`;
- canonical format: `<PROJECT_CODE>-CX-<NNNN> <task-slug>`;
- `kind: self_handoff`;
- status chain начинается с `codex_self_handoff`;
- self-handoff block должен содержать `codex_work_id`, `codex_work_title`, `task_slug`, `codex_work_kind`, `codex_work_state`, `codex_work_index_path`;
- bootstrap/launcher обязан записать item в `.chatgpt/codex-work-index.yaml` до видимого self-handoff ответа.

Запрещено расходовать `FT-CH-....` номера на Codex direct-task/self-handoff remediation: эти номера зарезервированы для стабильных ChatGPT chat titles.

## Handoff shape и фактический execution mode

`handoff_shape` фиксируется до выдачи handoff, но он не должен заранее объявлять фактическую оркестрацию.

Preferred value для новых handoff:
- `codex-task-handoff`.

Legacy values читаются для backward compatibility, но не являются preferred output:
- `single-agent-handoff`;
- `parent-orchestration-handoff`.

User-facing handoff всегда один. ChatGPT/handoff layer не должен заранее называть handoff "оркестровым". Codex после route receipt и анализа task graph выбирает actual execution mode:
- `single-session execution`;
- `orchestrated-child-sessions`.

В handoff можно и нужно фиксировать orchestration candidate signals, если они есть, но это только сигнал для runtime decision, а не факт исполнения.
...
Если hard trigger не сработал, но есть три или больше soft signals, пометь задачу как orchestration candidate, но не меняй `handoff_shape` с `codex-task-handoff`:
...
Если Codex после анализа выбирает `orchestrated-child-sessions`, он обязан материализовать/прочитать parent orchestration plan и зафиксировать:
...
Фактический closeout обязан отдельно назвать `execution_mode` и `child/subagent count`. Если child/subagent sessions не запускались, запрещено описывать результат как "оркестр агентов"; нужно писать, что была выполнена `single-session execution`, а child/subagent count равен `0`.

Запрещено:
- называть handoff оркестровым только потому, что задача важная;
- заранее обещать фактический `orchestrated-child-sessions`, пока Codex не решил запускать child/subagent sessions;
- скрывать orchestration candidate signals для большой задачи, где есть независимые child subtasks с разными профилями;
