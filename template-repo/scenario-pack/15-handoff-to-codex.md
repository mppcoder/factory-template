...
`parent-orchestration-handoff` — это выбранная форма handoff/plan. Фактический closeout обязан отдельно назвать execution mode и child/subagent count. Если child/subagent sessions не запускались, запрещено описывать результат как "оркестр агентов"; нужно писать, что была выполнена single-session remediation на parent route, а child/subagent count равен `0`.
