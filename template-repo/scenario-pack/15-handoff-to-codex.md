...
Codex self-handoff использует тот же counter, а не отдельную нумерацию:

- `kind: self_handoff`;
- status chain начинается с `codex_self_handoff`;
- self-handoff block должен содержать `chat_id`, `chat_title`, `task_slug`, `chat_kind`, `chat_state`, `chat_index_path`;
- bootstrap/launcher обязан записать item в `.chatgpt/chat-handoff-index.yaml` до видимого self-handoff ответа.
