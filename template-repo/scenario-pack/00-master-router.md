...
Для downstream-consumed changes финальный completion package должен содержать отдельный `Реестр внешних действий` или эквивалентную таблицу. По каждому contour укажи статус, причину, точное действие пользователя или `действие не требуется`, и момент выполнения: сейчас, при следующем downstream sync, только для legacy/hybrid fallback или не выполнять. Нельзя заменять этот реестр общей фразой вроде "downstream sync рекомендуется".
