...
Если нужны любые внешние действия пользователя, `Инструкция пользователю` оформляется как диалоговый сценарий: поясни, зачем нужен шаг, какое окно открыть, что нажать или вставить, какой вариант выбрать по умолчанию и что делать при типичной ошибке. Если нужно что-то копировать, выдай точный fenced code block прямо в ответе; ссылка на файл или путь в repo может быть только дополнительным evidence, но не заменой copy block.
...
- При любых внешних действиях реализуй сценарий диалога с подробными пояснениями шагов, рекомендованным вариантом и fallback-вариантом, если развилка реально есть.
