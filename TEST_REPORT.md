# Отчет TEST REPORT v2.5.6 package-ready

Status source of truth: `docs/releases/release-scorecard.yaml`.
Current scorecard state: `2.5.6 Package Ready`.
...
Добавлен прозрачный Windows-first путь установки `factory-template` с нуля на VPS: PowerShell entrypoint, remote bash installer, prompts для Codex/ChatGPT Project, README, future exe packaging boundary и targeted validator. Для `2.5.6` добавлен `WINDOWS_INSTALL_LATEST.md`: один файл для новичка со ссылками и copy-paste PowerShell блоками, который через GitHub API скачивает latest release ZIP/manifest/SHA256 без fixed version, проверяет checksum, распаковывает package и рекурсивно находит `windows-bootstrap/install-windows.ps1`. Сохранены existing-key check, PowerShell 7 recommendation, `winget` update command, safe defaults and passwordless SSH setup from previous patch releases. Recommended source остается GitHub clone/download из `mppcoder/factory-template`; release archive + manifest + SHA256 остается fallback; npm install path не поддерживается.
...
- `bash RELEASE_BUILD.sh /tmp/factory-v2.5.6-windows-bootstrap-test.zip` — pass; release ZIP, sidecar manifest and SHA256 produced and validated.
