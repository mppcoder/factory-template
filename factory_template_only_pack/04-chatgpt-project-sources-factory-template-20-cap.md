...
Внутри папки также есть:
- `UPLOAD_TO_SOURCES.txt` — что загружать в Sources;
- `DO_NOT_UPLOAD.txt` — что загружать не нужно.
...
- в Sources проекта загружать только файлы, перечисленные в `core-hot-15/UPLOAD_TO_SOURCES.txt`;
...
И не должен гадать по служебным файлам: `DO_NOT_UPLOAD.txt` явно показывает, что не нужно выгружать в Sources.
